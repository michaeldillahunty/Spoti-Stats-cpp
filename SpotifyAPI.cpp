...
/** 
 * REQUIRED: Function that encodes the client_id and client_secret into a base 64 string 
...
std::string SpotifyAPI::get_auth_token(const std::string& client_id, const std::string& client_secret){
   ...
   if (curl) {
      ...
      std::string data = "grant_type=client_credentials&scope=user-read-private%20user-read-email";
